pipeline {
    agent any
    environment {
        REPO = "https://github.com/Barsoup-Tensor/auth-server.git"  // Github repository URL
        DOCKER_IMAGE = "auth"  // 이미지 이름
        DOCKER_CONTAINER = "auth"  // 컨테이너 이름
    }
    stages {
        stage('Checkout') {
            steps {
        ...
        stage('Up') {
            steps {
                script {
                    ...
                    sh '''
                    docker run -d --name ${DOCKER_CONTAINER} -p 8050:8050 \
